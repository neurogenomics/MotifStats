#' Read motif file and convert to PWM
#'
...
#' @param motif_id ID of the motif (e.g. MA1930.1). The default is "Unknown"
#' @param file_format Character string specifying the format of the motif file.
#' The options are "jaspar", "meme", "transfac" and "uniprobe"
#'
#' @returns A position weight matrix.
#'
#' @export
read_motif_file <- function(motif_file,
  ...
  if (!file_format %in% names(read_functions)) {
    stop("Unsupported file format. The motif file must be one of",
         "jaspar, meme, transfac or uniprobe.")
  }
